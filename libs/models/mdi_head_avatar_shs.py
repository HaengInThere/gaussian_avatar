...
from libs.utils.general_utils   import inverse_sigmoid
from libs.utils.graphics_utils  import compute_vertex_normals
...
class MDIHeadAvatar(nn.Module):
    ...
    def _sample_uv_features_from(self, uv_maps, keys=None):
        ...
        def samp4(tex4, mask4=None):
            ...
            if mask4 is None:
                ...
                wei =  mask4[y_idx, x_idx]                         # (N,)
                out = (val / (wei.unsqueeze(0) + 1e-8)).transpose(0, 1).contiguous()
            return out  # (N,C)
    ...
    def forward_geometry(self, viewpoint_cam):
        ...
        # 1) UV valid mask를 저장해 이후 샘플링에 사용
        if isinstance(uv_valid_mask, torch.Tensor):
            self._uv_valid_mask = uv_valid_mask.unsqueeze(0).to(uv_base.device).contiguous()
        else:
            self._uv_valid_mask = None

        # 2) 0-벡터 normal을 +Z로 대체하고 정규화
        eps = 1e-6
        n = uv_base_normal
        mag = torch.linalg.norm(n, dim=1, keepdim=True)
        fallback = torch.tensor([0.0, 0.0, 1.0], device=n.device, dtype=n.dtype).view(1,3,1,1)
        n = torch.where(mag > eps, n, fallback.expand_as(n))
        uv_base_normal = F.normalize(n, dim=1, eps=eps)

        # 덮어쓰기
        uv_base = torch.cat([uv_base_pos, uv_base_normal, uv_flame_orient], dim=1)
    ...
    def build_static_gaussian(self, geom, uv_maps=None):
        ...
        sampled = self._sample_uv_features_from(uv_maps, 
                                                keys=["static_pos", "static_slog", "static_rot","static_normal"])
        gaussian._xyz = sampled['static_pos']

        # --- after ---
        LOG_S_MIN, LOG_S_MAX = -10.0, -0.1  # exp() 기준 ≈ [3.4e-4, 20.1]
        slog = sampled['static_slog'].clamp(LOG_S_MIN, LOG_S_MAX)
        gaussian._scaling = slog
        # gaussian._scaling = sampled['slog']

        gaussian._rotation = sampled['static_rot']

        gaussian._normal = sampled['static_normal']
        ...
        pos_smt = _smooth_loss_on_mask(uv_maps['static_pos'].abs()).mean(dim=0, keepdim=True)
        slog_smt = _smooth_loss_on_mask(uv_maps['static_slog'].abs()).mean(dim=0, keepdim=True)
        # rot_smt = _smooth_loss_on_mask(uv_maps['static_rot'].abs()).mean(dim=0, keepdim=True)
        normal_smt = _smooth_loss_on_mask(uv_maps['static_normal'].abs()).mean(dim=0, keepdim=True)
        ...
        # Prepare inputs
        pos_map  = uv_maps['static_pos']      # (B=1,3,H,W)
        ...
        area0_safe = torch.clamp(area0, min=1e-6)
        area_ratio = torch.log(area_safe / area0_safe)
        area_ratio = torch.clamp(area_ratio, -10, 10)
    ...
    def build_gaussian(self, geom, uv_maps=None):
        ...
        eps_floor = 0.01
        sampled_app = self._sample_uv_features_from(uv_maps, keys=["features_dc_logit", "opacity_logit"])
        # ===== Appearance =====
        # color logits
        color_logit = sampled_app['features_dc_logit']
        # opacity logits
        opacity_logit_raw = sampled_app['opacity_logit']
