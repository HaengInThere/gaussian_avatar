...
from libs.models.lbs import lbs, vertices2landmarks, blend_shapes, vertices2joints
from libs.utils.mesh import face_vertices
...
# FLAME_MODEL_PATH = "/hdd1/csg/src/MultiGSFace/assets/flame/generic_model.pkl"
FLAME_MODEL_PATH = "./assets/flame/flame2023.pkl"
...
FLAME_PAINTED_TEX_PATH = "./assets/flame/tex_mean_painted.png"
FLAME_UVMASK_PATH = "./assets/flame/uv_masks.npz"
...
class FlameHead(nn.Module):
    ...
    def forward(
        ...
        v_shaped = template_vertices + blend_shapes(betas, shapedirs)
        ...
        if delta_vertex is None:
            ...
            v_shaped += delta_vertex.unsqueeze(0).expand(batch_size, -1, -1)
...
if __name__ == '__main__':
    ...
    shape = torch.zeros((1,300)).float().cuda()  # Neutral shape
    expr = torch.zeros((1, 100)).float().cuda()   # No expression
    rotation = torch.zeros((1, 3)).float().cuda()  # No rotation
    neck_pose = torch.zeros((1, 3)).float().cuda() # No neck movement
    jaw_pose = torch.zeros((1, 3)).float().cuda()  # No jaw movement
    eyes_pose = torch.zeros((1, 6)).float().cuda() # No eye movement
    translation = torch.zeros((1, 3)).float().cuda()  # No translation
    static_offset = torch.zeros(size=(1,5143,3)).float().cuda()
    ...
    faces = flame_model.faces # 9976 3 
    faces = faces.unsqueeze(0).repeat(1, 1, 1) 
    
    verts_uv = flame_model.verts_uvs
    faces_uv = flame_model.textures_idx
    
    verts = verts[0].cpu()
    faces = faces[0].cpu()
    verts_uv = verts_uv.cpu()
    faces_uv = faces_uv.cpu()
    print(verts.shape, faces.shape, verts_uv.shape, faces_uv.shape)

    # Save the OBJ with UVs
    save_obj(
        "/hdd1/csg/src/MultiGSFace/assets/flame/head_template_mesh_vhap.obj",
        verts=verts,  # Vertex positions (Nx3)
        faces = faces,  # Face indices (Fx3)
        verts_uvs=verts_uv,  # UV coordinates (Nx2)
        faces_uvs=faces_uv,  # UV face indices (Fx3)
    )

    # save_obj(f"/hdd1/csg/src/MultiGSFace/assets/flame/head_template_mesh_vhap.obj", verts[0], faces[0])
