...
def render(
    ...
    means3D = pc.get_xyz.to(device)
    means2D = screenspace_points.to(device)
    opacity = pc.get_opacity.to(device)
    normal = pc.get_normal.to(device)
    albedo = pc.get_albedo.to(device)
    roughness = pc.get_roughness.to(device)
    metallic = pc.get_metallic.to(device)
