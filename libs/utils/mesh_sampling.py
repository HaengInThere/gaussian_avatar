...
def uniform_sampling_barycoords(
    ...
    pix_to_face, bary_coords = uv_rasterizer(tex_coord_.expand(1, -1, -1), uv_faces.expand(1, -1, -1)) # 1 256 256 1 / 1 256 256 1 3
    mask = (pix_to_face == -1) # 65,536 중 -1 인것들 masking 

    face_index = pix_to_face[~mask] # 59099 -1이 아닌 각 가우시안 포인트가 위치한 face의 index 
    bary_coords = bary_coords[~mask] # 59099 3 
    ...
    return face_index, bary_coords # 65536
...
def reweight_verts_by_barycoords(
    ...
    face_index_3    = face_index.view(1, N, 1, 1).expand(B, N, 3, 3) # 1 65536 3 3 
    # 각 가우시안의 face index를 기준으로 3d vertex 좌표를 가져옴 
    position_vals   = face_verts.gather(1, face_index_3) # 1 65536 3 3 
    # 삼각형 내부의 가우시안 위치를 bary centric 으로 찾음 
...
def sample_uvmap_by_barycoords(
    uv_map: torch.Tensor,        # (C,H,W) or (1,C,H,W)
    uvcoords: torch.Tensor,      # (V_uv, 2) in [0,1]
    uvfaces: torch.Tensor,       # (F,3)
    face_index: torch.Tensor,    # (N,)
    bary_coords: torch.Tensor,   # (N,3)
    *,
    flip_v: bool = True,         # unwrap에서 V를 뒤집었으면 True
    align_corners: bool = True,  # unwrap과 동일 값으로
) -> torch.Tensor:
    """
    UV map에서 barycentric 정보(face_index, bary_coords)로 N개 지점의 값을 샘플링.
    bilinear가 아닌 '픽셀 최근접'으로 읽어 래스터 결과와 수치 일치시키기.
    반환: (N, C)
    """
    device = uv_map.device
    face_index = face_index.long().to(device)
    bary_coords = bary_coords.to(device)

    # 1) face별 3개 UV 좌표 취득 (N,3,2) in [0,1]
    tri_uv = uvcoords.to(device)[ uvfaces[face_index] ]  # (N,3,2)

    # 2) barycentric으로 연속 uv 좌표 (N,2)
    uv = (bary_coords.unsqueeze(-1) * tri_uv).sum(dim=1)  # (N,2) in [0,1]

    # 3) unwrap과 동일한 축 플립 적용
    if flip_v:
        uv = uv.clone()
        uv[:, 1] = 1.0 - uv[:, 1]

    # 4) 연속 uv → 픽셀 인덱스로 스냅 (Nearest)
    if uv_map.dim() == 3:
        C, H, W = uv_map.shape
        uv_map_bchw = uv_map.unsqueeze(0)  # (1,C,H,W)
    else:
        _, C, H, W = uv_map.shape
        uv_map_bchw = uv_map

    if align_corners:
        x = torch.round(uv[:, 0] * (W - 1)).long()
        y = torch.round(uv[:, 1] * (H - 1)).long()
    else:
        # PyTorch grid_sample의 align_corners=False 규약
        x = torch.round(uv[:, 0] * W - 0.5).long()
        y = torch.round(uv[:, 1] * H - 0.5).long()

    x = x.clamp(0, W - 1)
    y = y.clamp(0, H - 1)

    # 5) 픽셀 인덱싱으로 그대로 읽기 (Nearest)
    sampled = uv_map_bchw[0, :, y, x].transpose(0, 1).contiguous()  # (N,C)
    return sampled
